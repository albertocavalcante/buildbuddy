...
func TestDistributedCache(t *testing.T) {
	...
	for i := 0; i < clusterSize; i++ {
		ddiskPorts = append(ddiskPorts, testport.FindFree(t))
	}
	...
	for i := 0; i < clusterSize; i++ {
		flags := []string{
			...
			"--cache.distributed_cache.listen_addr=" + fmt.Sprintf("localhost:%d", ddiskPorts[i]),
			"--app.log_level=info", // DO NOT SUBMIT (switch to warn)
			"--zone_override=test",
		}
		for _, port := range ddiskPorts {
	...
	// TODO: is there a way to do this explicitly?
		...
		eg.Go(func() error {
			...
			if shutdownDuringWrites && i == numWrites/2 {
				go func() {
					...
					log.Infof("Test: shutting down app")
					cluster.ShutdownApp(rand.Intn(len(cluster.Apps)))
				}()
			}

			log.Infof("Test: calling UploadFromReader")
